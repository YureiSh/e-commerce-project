        ...
        <footer className='footer'>
            <div className='max-w-360 m-auto'>
                <div className='flex flex-col lg:flex-row items-baseline lg:items-center justify-between'>
                    <div><h3 className='bandage cursor-default' >Bandage</h3></div>
                    <div className='flex gap-3 cursor-pointer hover:bg-gray-100' >
                        <FaFacebook size={18} className='text-primary customImg' href='#' />
                        <FaInstagram size={18} className='text-primary customImg' href='#' />
                        <FaTwitter size={18} className='text-primary customImg' href='#' />
                    </div>
                </div>
                ...
                <div className='max-w-360 flex flex-col lg:flex-row m-auto justify-between px-4 lg:px-0 pt-16 pb-16 gap-16'>
                    {footerData.map((section) => (
                        <div key={section.title}>
                            <h3 className="font-bold text-secondary mb-3 text-xl cursor-default">{section.title}</h3>

                            <ul className='flex flex-col gap-2'>
                                {section.links.map((link) => (
                                    <li key={link.label} className='font-bold text-[#737373]' >
                                        <Link className="basicHover" href={link.href}>{link.label}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
